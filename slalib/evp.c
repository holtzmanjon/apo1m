...
/* Secular perturbations in longitude */
   for ( k = 0; k < 4; k++ ) {
      a = fmod ( ccsec[1][k] + dt * ccsec[2][k] , D2PI );
      sn[k] = sin ( a );
   }
...
/* Periodic perturbations of the EMB (Earth-Moon barycentre) */
   ...
   pertrd = 0.0;
   for ( k = 0; k < 15; k++ ) {
      a = fmod ( dcargs[0][k] + dt * dcargs[1][k] , D2PI );
      cosa = cos ( a );
      sina = sin ( a );
...
** geocentric motion of the Moon */
   ...
   pertpd = 0.0;
   for ( k = 0; k < 3; k++ ) {
      a = fmod ( dcargm[0][k] + dt * dcargm[1][k] , D2PI );
      sina = sin ( a );
      cosa = cos ( a );
...
/* Barycentric motion of the Earth */
   ...
   for ( k = 0; k < 4; k++ ) {
      ...
      pomg = sorbel[k+1];
      pecc = sorbel[k+9];
      tl = fmod( plon + 2.0 * pecc * sin ( plon - pomg ) , D2PI );
      sinlp[k] = sin ( tl );
      coslp[k] = cos ( tl );
