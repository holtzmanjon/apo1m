...
do i=1,nfilt
  ...
  texp={var}
  ! Use highest numbered filter with t>0 for focus
  if abs[texp]>0
    focfilt=i
  end_if
  ! If SDSS filters, use g for acquisition
  if abs[texp]>0&i>6
    cfilt=7 
  end_if
end_do
! dont use U for focus
!if focfilt==1
!  focfilt=2
!end_if
! Type of focus star, SAFOC for U, else USNO
if focfilt==6|focfilt==1
  saotype=2
else
  saotype=0
end_if
...
if getguide>0
  ...
  xs=20 xe=ncol-20 ys=20 ye=nrow-20
end_if
box 10 nr=ye-ys nc=xe-xs sr=ys sc=xs
get mask=./guide.msk
