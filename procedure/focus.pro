...
do ii=start,end
  ...
  if (noauto~=2)&(ii==start|noauto==1)
    ...
    if noauto==2
      automark $i auto id=ii-start+1 radius=radius
    else
  ...
  fwavg=99
  starplot $i scale=1 phot gauss {noplot} load   !silent
  ...
  end_if
  if fwavg<fwmin
    fwmin=fwavg minfoc={i:{card}}
  end_if
!  printf 'high: %2f8.3' h/tot high
!  printf 'ax: %2f8.3' axfw axmin
!  printf 'min: %2f8.3' fwavg fwmin
end_do

! Do overall fit to image fwhms
get phot=./focus
string psfile 'focus%i3.3' start
plotfocus scale=scale focus=focus fwhm=fwhm hard psfile=./{psfile}.ps
$cp {psfile}.ps lastfocus.ps
!plotfocus scale=scale focus=focus fwhm=fwhm {noplot}
