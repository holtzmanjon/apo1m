...
call startup
ask 'Enter 1 for NA1/Leach or 2 for NA2/APOGEE: ' port
if port==1
  scidet=32
  inst=1
else
...
if port==2
  printf ' You need to start the APOGEE actor. Hit C when ready '
  pause
end_if
...
if daytest==0&localtim<(sunset-0.75)&localtim>ntwimorn
  ...
  !sleep=(sunset+0.5-localtim)*3600
end_if
if sleep>0
...
call time
if (localtim>12&localtim<ntwieve-0.75&daytest==0)
...
send '-alert'
printf 'twimorn'
call twiwait ntwimorn 0.25
call time
if localtim<12&localtim<ntwimorn+0.85
  call readstat 1
  if domeopen==1&scidet~=17
!    call flatmorn 5
    call flatmorn 
  end_if
end_if
