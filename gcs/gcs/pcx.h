...
extern void pcx_get_axis_status(const byte card, const char axis,
																struct AXISPACK &axisData);
extern byte pcx_read_byte(const byte card);
...
extern unsigned pcx_read_inputs(const byte card);
extern unsigned pc38_read_inputs();
...
inline void pc34_get_axis_status(const char axis,
																struct AXISPACK &axisData)
	{ pcx_get_axis_status(PC34, axis, axisData); }

inline byte pc34_read_byte()
	{ return pcx_read_byte(PC34); }
...
inline unsigned pc34_read_inputs()
        { return pcx_read_inputs(PC34); }
...
inline void pc38_get_axis_status(const char axis,
																struct AXISPACK &axisData)
	{ pcx_get_axis_status(PC38, axis, axisData); }

inline byte pc38_read_byte()
	{ return pcx_read_byte(PC38); }
...
//inline unsigned pc38_read_inputs()
//        { return pcx_read_inputs(PC38); }
#endif
/********************************* EOF **************************************/
