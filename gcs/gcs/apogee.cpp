...
// Get camera information
  ...
  sprintf(outbuf,"cooler set point: %d\n"
         "binning factors: %d %d\n"
         "bits per color: %d\n"
         "number of colors: %d\n"
         "x size: %d\n"
...
ccd_write(WORD **buf, unsigned int exptime, int nc, int nr, int sc, int sr, int ndark)
{
  ...
  if (guiding)  {
    if (writeguide) {
      sprintf(card,"t%07d.spe",iseq++);
    }
  }
  else if (ndark<0) {
    sprintf(card,"temp.spe");
  } else {
    sprintf(card,"dark%1d.spe",ndark);
  }
      ...
      int update, double ax, double bx, double ay, double by)
{
  ...
  float row, col, peak, tot, totsave[50];
