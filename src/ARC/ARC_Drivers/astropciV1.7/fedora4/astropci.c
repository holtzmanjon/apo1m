...
static unsigned int astropciReadl(long addr);
static unsigned short astropciReadw(long addr);
static void astropciWritel(unsigned int value, long addr);
static void astropciWritew(unsigned short value, long addr);
...
{
        {PCI_VENDOR_ID_MOTOROLA, ARC_PCI_DEVICE_ID, PCI_ANY_ID, PCI_ANY_ID, 0, 0, 0},
        {0,}
};
...
/* This was used with the old way of declaring the board array.
...
#if 0
		/*
		 * Initialize the /proc filesytem info. This sets a pointer to the
		 * function that will be called when someone reads /proc/astropci#.
		 * The # represents the device number, and is passed to the function.
		 * For example: cat /proc/astropci0
		 */
		astropciProcFile = create_proc_read_entry(board[i], 0, NULL, astropci_read_proc, (void *)i);
		astropciProcFile->owner = THIS_MODULE;
		astropciProcFile->uid = 0;
		astropciProcFile->gid = 0;

		if (astropciProcFile == NULL) {
			remove_proc_entry(board[i], &proc_root);
			unregister_chrdev(major[i], board[i]);
                	major[i] = 0;
			astropci_printf("Error: Could not initialize /proc/%s\n", board[i]);
			return -ENOMEM;
		}
		else {
			astropci_printf("Success! /proc/%s initialized.\n", board[i]);
		}
#endif
...
//                	remove_proc_entry(board[i], &proc_root); //(board[i], NULL);
			...
			if (ret < 0)
				astropci_printf("Error in unregister_chrdev: %d\n", ret);
                ...
                const struct pci_device_id *ent)
{
	...
	/* Initialize the board names array for later use */
//	sprintf((char *)&board[dev][0], "%s%d", DRIVER_NAME, dev);
//	astropci_printf("board[%d][]: %s\n", dev, board[dev]);
...
astropci_open(struct inode *inode, struct file *file)
{
        ...
        value = astropciReadl(astropci[dev].ioaddr + HCTR);
        astropciWritel(((value & 0xCFF) | 0x900), astropci[dev].ioaddr + HCTR);
 ...
 exit:
	...
	/* Increase the module usage count. Prevents accidental unloading
	   of the device while it's in use. */
               ...
               unsigned long arg)
{
        ...
        case ASTROPCI_GET_HCTR:
                reg = astropciReadl(astropci[dev].ioaddr + HCTR);
        ...
        case ASTROPCI_GET_PROGRESS: {
                ...
                astropciWritel((uint32_t) READ_PCI_IMAGE_ADDR, astropci[dev].ioaddr + HCVR);

                /* Read the current image address */
                if (astropci_check_dsp_output_fifo(dev)) {
                        lower = astropciReadw(astropci[dev].ioaddr + REPLY_BUFFER);
                        upper = astropciReadw(astropci[dev].ioaddr + REPLY_BUFFER);
        ...
        case ASTROPCI_GET_FRAMES_READ: {
  		...
  		/* Ask the PCI board for the current image address */
   		astropciWritel ((uint32_t)READ_NUMBER_OF_FRAMES_READ, astropci[dev].ioaddr + HCVR);
        ...
        case ASTROPCI_GET_HSTR:
                reg = astropciReadl(astropci[dev].ioaddr + HSTR);
                if (put_user(reg, (uint32_t *) arg))
                        retval = -EFAULT;
                break;
        ...
        case ASTROPCI_SET_HCTR:
                if (get_user(reg, (uint32_t *) arg)) {
                        ...
                        astropciWritel(reg, astropci[dev].ioaddr + HCTR);
                }
                break;
        ...
        case ASTROPCI_SET_HCVR: {
                ...
                if (get_user(Hcvr, (uint32_t *) arg))
                        ...
                        if (Hcvr != ABORT_READOUT)
                                astropciWritel((uint32_t) CLEAR_REPLY_FLAGS,
                                       astropci[dev].ioaddr + HCVR);

                        /* Pass the command to the PCI board */
                        astropciWritel(Hcvr, astropci[dev].ioaddr + HCVR);
                        ...
                        if (reply != RDR) {
                                ...
                                /* Go read some data */
                                astropciWritel((uint32_t) READ_REPLY_VALUE,
                                       astropci[dev].ioaddr + HCVR);
                                if (astropci_check_dsp_output_fifo(dev)) {
                                        reply = astropciReadl(astropci[dev].ioaddr + REPLY_BUFFER);
                                        if (put_user(reply, (uint32_t *) arg))
                                                retval = -EFAULT;
                                } else
                                        retval = -EFAULT;
        ...
        case ASTROPCI_HCVR_DATA: {
                ...
                if (get_user(Cmd_data, (uint32_t *) arg))
                        ...
                        if (astropci_check_dsp_input_fifo(dev))
                                astropciWritel(Cmd_data, astropci[dev].ioaddr + CMD_DATA);
                        else
                                retval = -EIO;
                }
        }
		...
		int numberOfParams = 0;
                ...
                if (copy_from_user(Cmd_data, (uint32_t *) arg, sizeof (Cmd_data)))
                	retval = -EFAULT;
                        ...
                        astropciWritel((uint32_t) CLEAR_REPLY_FLAGS, astropci[dev].ioaddr + HCVR);
				...
				if (numberOfParams > CMD_MAX) {
					...
					break;
                                ...
                                for (i = 0; i < numberOfParams; i++)
                                         astropciWritel(Cmd_data[i], astropci[dev].ioaddr + CMD_DATA);

                                /* Tell the PCI board to do a WRITE_COMMAND vector command */
                                astropciWritel((uint32_t) WRITE_COMMAND, astropci[dev].ioaddr + HCVR);
                                ...
                                if (Cmd_data[0] != RDR) {
                                        ...
                                        /* Go read some data */
                                        astropciWritel((uint32_t) READ_REPLY_VALUE, astropci[dev].ioaddr + HCVR);
                                        if (astropci_check_dsp_output_fifo(dev)) {
                                                Cmd_data[0] = astropciReadl(astropci[dev].ioaddr + REPLY_BUFFER);
                                                if (copy_to_user((uint32_t *) arg, Cmd_data, sizeof (Cmd_data)))
                                                        retval = -EFAULT;
                                        } else {
                                                retval = -EFAULT;
        ...
        case ASTROPCI_PCI_DOWNLOAD:
                ...
                astropciWritel((uint32_t) PCI_DOWNLOAD, astropci[dev].ioaddr + HCVR);
                break;
...
astropci_intr(int irq, void *dev_id, struct pt_regs *regs)
{
        ...
        if (dev >= 0 && dev < MAX_DEV && astropci[dev].have_irq) {
                int_flag = astropciReadl(astropci[dev].ioaddr + HSTR);
        }
...
*/
	...
	astropciWritel((uint32_t) CLEAR_INTERRUPT, astropci[dev].ioaddr + HCVR);
...
astropci_set_buffer_addresses(int dev)
{
        ...
        if (astropci_check_dsp_input_fifo(dev)) {
                ...
                astropciWritew((uint16_t) (phys_address & 0x0000FFFF),
                       astropci[dev].ioaddr + CMD_DATA);
                astropciWritew((uint16_t) ((phys_address & 0xFFFF0000) >> 16),
                       astropci[dev].ioaddr + CMD_DATA);
        } else {
        ...
        astropciWritel((uint32_t) WRITE_PCI_ADDRESS, astropci[dev].ioaddr + HCVR);
...
astropci_flush_reply_buffer(int dev)
{
        ...
        for (x = 0; x < 6; x++) {
                reply_status = astropciReadl(astropci[dev].ioaddr + HSTR);
                if ((reply_status & OUTPUT_FIFO_OK_MASK) == OUTPUT_FIFO_OK_MASK)
                        reply_value = astropciReadl(astropci[dev].ioaddr + REPLY_BUFFER);
                else
                        break;
        }
...
static int astrocpi_check_for_readout(int dev)
{
	uint32_t status = astropciReadl(astropci[dev].ioaddr + HSTR);
...
astropci_wait_for_condition(int dev, int condition_to_wait_for)
{
        ...
        while (elapsed_time < SLEEP_MAX_WAIT) {
                status = astropciReadl(astropci[dev].ioaddr + HSTR);
...
astropci_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if ((retval = remap_pfn_range(vma,
                                      vma->vm_start,
				      (astropci[dev].imageBufferStart >> PAGE_SHIFT),
