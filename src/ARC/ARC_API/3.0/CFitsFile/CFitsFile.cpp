...
// |  CFitsFile.cpp                                                            |
// +---------------------------------------------------------------------------+
// |  Defines the exported functions for the CFitsFile DLL.  Wraps the cfitsio |
...
// |  Copy constructor                                                         |
// +---------------------------------------------------------------------------+
// |  Copies an existing CFitsFile, headers only.  The image data is not       |
// |  copied.                                                                  |
// |                                                                           |
...
// +---------------------------------------------------------------------------+
CFitsFile::CFitsFile( CFitsFile& rAnotherFitsFile, const char* pszNewFileName )
{
	...
	// +------------------------------------------------------------+
	fits_copy_header( rAnotherFitsFile.GetBaseFile(), m_fptr, &dFitsStatus );
...
// |  <IN> -> szComment - The comment to attach to the keyword.  Can be NULL.  |
// +----------------------------------------------------------------------------
void CFitsFile::WriteKeyword( char* pszKey, void* pKeyVal, int dValType, char* szComment )
{
    int dFitsStatus = 0;
...
// +----------------------------------------------------------------------------
void CFitsFile::UpdateKeyword( char* pszKey, void* pKeyVal, int dValType, char* szComment )
{
	WriteKeyword( pszKey, pKeyVal, dValType, szComment );
}
...
void CFitsFile::GetParameters( long* pNaxes, int* pNaxis, int* pBitsPerPixel )
{
	...
	int  dTempBPP        = 0;
	int  dTempNAXIS      = 0;
	long lTempNAXES[ CFitsFile::NAXES_SIZE ] = { 0, 0, 0 };
	...
	if ( pNaxes != NULL )
	{
		pNaxes[ CFitsFile::NAXES_COL ] = lTempNAXES[ CFitsFile::NAXES_COL ];
		pNaxes[ CFitsFile::NAXES_ROW ] = lTempNAXES[ CFitsFile::NAXES_ROW ];
		pNaxes[ CFitsFile::NAXES_NOF ] = lTempNAXES[ CFitsFile::NAXES_NOF ];
	}
...
// |  returns the CFitsFile::NAXES_NOF element in the pNaxes array to the      |
...
// |  Throws std::runtime_error on error.                                      |
// +---------------------------------------------------------------------------+
long CFitsFile::GetNumberOfFrames()
{
	long lNAxes[ CFitsFile::NAXES_SIZE ] = { 0, 0, 0 };

	GetParameters( &lNAxes[ 0 ] );

	return lNAxes[ CFitsFile::NAXES_NOF ];
}
...
// |  returns the CFitsFile::NAXES_ROW element in the pNaxes array to the      |
// |  method GetParameters().  To get all parameters ( rows, cols,             |
// |  number-of-frames, naxis value and bits-per-pixel ) at one time ... call  |
// |  GetParameters() instead.                                                 |
// |                                                                           |
// |  Throws std::runtime_error on error.                                      |
// +---------------------------------------------------------------------------+
long CFitsFile::GetRows()
{
	long lNAxes[ CFitsFile::NAXES_SIZE ] = { 0, 0, 0 };

	GetParameters( &lNAxes[ 0 ] );

	return lNAxes[ CFitsFile::NAXES_ROW ];
}
...
// |  returns the CFitsFile::NAXES_COL element in the pNaxes array to the      |
// |  method GetParameters().  To get all parameters ( rows, cols,             |
// |  number-of-frames, naxis value and bits-per-pixel ) at one time ... call  |
// |  GetParameters() instead.                                                 |
// |                                                                           |
// |  Throws std::runtime_error on error.                                      |
// +---------------------------------------------------------------------------+
long CFitsFile::GetCols()
{
	long lNAxes[ CFitsFile::NAXES_SIZE ] = { 0, 0, 0 };

	GetParameters( &lNAxes[ 0 ] );

	return lNAxes[ CFitsFile::NAXES_COL ];
}
...
// +---------------------------------------------------------------------------+
fitsfile* CFitsFile::GetBaseFile()
{
	return m_fptr;
}
