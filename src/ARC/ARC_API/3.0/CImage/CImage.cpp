									 ...
									 int dRows, int dCols, int& dElemCount )
{
	...
	VERIFY_ROW( "GetImageRow", dRow,  dRows )
	VERIFY_COL( "GetImageRow", dCol1, dCols )
	VERIFY_COL( "GetImageRow", dCol2, dCols )

	VERIFY_RANGE_ORDER( "GetImageRow", dCol1, dCol2 )

	VERIFY_IMG_DIMEN( "GetImageRow", dRows );
	VERIFY_IMG_DIMEN( "GetImageRow", dCols );
									 ...
									 int dRows, int dCols, int& dElemCount )
{
	...
	VERIFY_ROW( "GetImageCol", dCol,  dCols )
	VERIFY_COL( "GetImageCol", dRow1, dRows )
	VERIFY_COL( "GetImageCol", dRow2, dRows )

	VERIFY_RANGE_ORDER( "GetImageCol", dRow1, dRow2 )

	VERIFY_IMG_DIMEN( "GetImageCol", dRows );
	VERIFY_IMG_DIMEN( "GetImageCol", dCols );
	...
	if ( pMem != NULL )
	{
		unsigned short *pU16Mem =
					static_cast<unsigned short *>( pMem );

		try
		{
			pU16Buf =
					new unsigned short[ size_t( dElemCount ) ];
		}
		catch ( bad_alloc& ba )
		{
							    ...
							    int dCol2, int dRows, int dCols, int& dElemCount )
{
	...
	VERIFY_ROW( "GetImageRowArea", dRow1, dRows );
	VERIFY_ROW( "GetImageRowArea", dRow2, dRows );
	VERIFY_COL( "GetImageRowArea", dCol1, dCols );
	VERIFY_COL( "GetImageRowArea", dCol2, dCols );

	VERIFY_RANGE_ORDER( "GetImageRowArea", dCol1, dCol2 );
	VERIFY_RANGE_ORDER( "GetImageRowArea", dRow1, dRow2 );

	VERIFY_IMG_DIMEN( "GetImageRowArea", dRows );
	VERIFY_IMG_DIMEN( "GetImageRowArea", dCols );
							    ...
							    int dCol2, int dRows, int dCols, int& dElemCount )
{
	...
	VERIFY_ROW( "GetImageColArea", dRow1, dRows );
	VERIFY_ROW( "GetImageColArea", dRow2, dRows );
	VERIFY_COL( "GetImageColArea", dCol1, dCols );
	VERIFY_COL( "GetImageColArea", dCol2, dCols );

	VERIFY_RANGE_ORDER( "GetImageColArea", dCol1, dCol2 );
	VERIFY_RANGE_ORDER( "GetImageColArea", dRow1, dRow2 );

	VERIFY_IMG_DIMEN( "GetImageColArea", dRows );
	VERIFY_IMG_DIMEN( "GetImageColArea", dCols );
...
// +----------------------------------------------------------------------------
CImage::CImgDifStats
CImage::GetDiffStats( void *pMem1, void *pMem2, int dRows, int dCols, int dBpp )
{
...
void CImage::SubtractImageHalves( void* pMem, int dRows, int dCols )
{
	if ( ( dRows % 2 ) != 0 )
	{
		...
		ThrowException( "SubtractImageHalves", oss.str() );
	}
...
void CImage::VerifyImageAsSynthetic( void* pMem, int dRows, int dCols )
{
	...
	if ( pU16Buf == NULL )
	{
		ThrowException( "VerifyImageAsSynthetic",
						"Invalid image memory pointer parameter!" );
	}
	...
	if ( dRows <= 0 )
	{
		...
		ThrowException( "VerifyImageAsSynthetic", oss.str() );
	}
