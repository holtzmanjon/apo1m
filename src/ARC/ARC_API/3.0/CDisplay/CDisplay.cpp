...
CDisplay::CDisplay()
{
	...
	char *cXpaVar = getenv( "XPA_METHOD" );

	if ( cXpaVar == NULL )
	{
		setenv( "XPA_METHOD", "local", 1 );
	}
	...
	if ( m_xpa == NULL )
	{
		ThrowException( "CDisplay",
				"Failed to open connection to XPA display server!" );
	}
#endif
}
...
// +---------------------------------------------------------------------------+
void CDisplay::Launch( int dMSDelay )
{
#ifdef WIN32
	RunAndSaveProcess( "ds9.exe" );
#else
	RunAndSaveProcess( "ds9" );
#endif

#if defined( linux ) || defined( __linux ) || defined( __APPLE__ )
	//
	// LINUX ONLY: Sleep for 1 sec to allow DS9 to start
	//
...
// +---------------------------------------------------------------------------+
void CDisplay::Show( void *pBuffer, int dRows, int dCols )
{
	ostringstream oss;

	oss << "array [xdim=" << dCols << ",ydim=" << dRows
	    << ",zdim=1,bitpix=16]" << ends;

	Set( oss.str(), pBuffer, dRows, dCols );
}
...
// +---------------------------------------------------------------------------+
void CDisplay::Show( std::string sFitsFile )
{
	ostringstream oss;

	oss << "file \"{" << sFitsFile << "}\"" << ends;
...
// +---------------------------------------------------------------------------+
void CDisplay::Clear( int dFrame )
{
	ostringstream oss;

	oss << "frame clear ";

	( ( dFrame > 0 ) ? oss << dFrame : oss << "all" );
...
// | Set                                                                       |
// +---------------------------------------------------------------------------+
// |  Executes the XPASet command with the specified DS9 command. The          |
// |  additional arguments are for displaying raw image data.                  |
// |                                                                           |
// |  NOTE: See the comment at the end of this file for a list of DS9 commands |
// |        Up-to-date version can be found here:                              |
// |        http://hea-www.harvard.edu/RD/ds9/ref/xpa.html                     |
// |                                                                           |
// |  Throws std::runtime_error on input parameter error or xpa error.         |
// |                                                                           |
// |  <IN> -> sDS9Cmd - DS9 command string.                                    |
// |  <IN> -> pBuffer - Image data buffer to display.                          |
// |  <IN> -> dRows   - Number of rows in image buffer.                        |
// |  <IN> -> dCols   - Number of columns in image buffer.                     |
// +---------------------------------------------------------------------------+
void CDisplay::Set( std::string sDS9Cmd, void *pBuffer, int dRows, int dCols )
{
#ifdef WIN32

	RunProcess( std::string( "xpaset -p ds9 " ) + sDS9Cmd );
	...
	if ( m_xpa != NULL )
	{
		int dGot = XPASet( m_xpa,
						   const_cast<char *>( "ds9" ),
						   const_cast<char *>( sDS9Cmd.c_str() ),
						   NULL,
						   ( char * )pBuffer,
						   ( dRows * dCols * sizeof( unsigned short ) ),
						   cNames,
						   cMessages,
						   NXPA );
		...
		if ( dGot > 0 )
		{
			...
			for( int i=0; i<dGot; i++ )
			{
				...
				if( cNames[ i ] )    { free( cNames[ i ] );    }
				if( cMessages[ i ] ) { free( cMessages[ i ] ); }
			}

			ostr << ends;

			if ( !( std::string( ostr.str().c_str() ) ).empty() )
			{
				ThrowException( "Set", ostr.str() );
			}
		}
	}
	else
	{
		ThrowException( "Set", "No DS9 connection exists!" );
	}
...
// +---------------------------------------------------------------------------+
// | RunProcess ( WIN32 ONLY )                                                 |
// +---------------------------------------------------------------------------+
// |  Creates a new WIN32 process.                                             |
// |                                                                           |
// |  <IN> -> sBuf - Process string to execute.                                |
// +---------------------------------------------------------------------------+
void CDisplay::RunProcess( std::string sBuf )
{
#ifdef WIN32
...
// | RunAndSaveProcess                                                         |
// +---------------------------------------------------------------------------+
// |  Creates a new WIN32 process and saves PROCESS_INFORMATION.               |
// |                                                                           |
// |  <IN> -> sBuf - Process string to execute.                                |
// +---------------------------------------------------------------------------+
void CDisplay::RunAndSaveProcess( std::string sBuf )
{
#ifdef WIN32
...
// +---------------------------------------------------------------------------+
#ifdef WIN32
bool CDisplay::GetModuleDirectory( TCHAR* pszBuf, size_t tBufSize )
{
	if ( !GetModuleFileName( 0, pszBuf, tBufSize ) )
    {
...
#else
bool CDisplay::GetModuleDirectory( char* pszBuf, size_t tBufSize )
{
	Dl_info	rInfo;
	...
	*pszBuf = 0;
	...
	if ( !dladdr( ( void * )&arc::CDisplay::GetModuleDirectory, &rInfo ) || !rInfo.dli_fname )
	{
		return false;
	}
...
// +----------------------------------------------------------------------------
void CDisplay::ThrowException( std::string sMethodName, std::string sMsg )
{
	throw std::runtime_error( string( "( CDisplay::" ) + 
