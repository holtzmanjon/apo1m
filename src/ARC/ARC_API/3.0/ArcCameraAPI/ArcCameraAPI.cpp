...
ArcCam_GetDeviceList( struct ArcCAPIDevList* pDevList, struct ArcCAPIStatus* pStatus )
{
	...
	//
	//  Clear the user device list
	// -------------------------------------------------
	pDevList->dDevCount = 0;
	memset( pDevList->szDevList, 0, ( DEV_LIST_SIZE * DEV_NAME_SIZE ) );
...
ArcCam_Set2xFOTransmitter( int dOnOff, struct ArcCAPIStatus* pStatus )
{
	...
	TRY_CATCH(
		pCArcDev.get()->Set2xFOTransmitter( ( dOnOff != 0 ) ), pStatus )
}
					    ...
					    const char* pszUtilFile, const char* pszPciFile, int* pAbort, struct ArcCAPIStatus* pStatus )
{
	...
	TRY_CATCH(
			pCArcDev.get()->SetupController( ( dReset != 0 ),
										     ( dTdl != 0 ),
											 ( dPower != 0 ),
											 dRows,
											 dCols,
...
ArcCam_SetSyntheticImageMode( int dMode, struct ArcCAPIStatus* pStatus )
{
	...
	TRY_CATCH(
		pCArcDev.get()->SetSyntheticImageMode( ( dMode != 0 ) ),
		pStatus )
}
...
ArcCam_SetOpenShutter( int dShouldOpen, struct ArcCAPIStatus* pStatus )
{
	...
	TRY_CATCH(
		pCArcDev.get()->SetOpenShutter( ( dShouldOpen != 0 ) ),
		pStatus )
}
...
ARCCAM_API void
ArcCam_Continuous( int dRows, int dCols, int dNumOfFrames, float fExpTime, int* pAbort,
...
ArcCam_SetLogCmds( int dOnOff )
{
	if ( pCArcDev.get() != NULL )
	{
		pCArcDev.get()->SetLogCmds( ( dOnOff != 0 ) );
	}
}
