...
CArcDisplay::CArcDisplay()
{
	...
	m_xpa = XPAOpen( nullptr );

	if ( m_xpa == nullptr )
	{
		ThrowException( "CArcDisplay",
...
void CArcDisplay::Launch( int dMSDelay )
{
	...
	if ( pszEnv == nullptr )
	{
		...
		oss << "Failed to locate DS9 path environment variable. "
			<< "Set \"DS9_HOME\" to location of ds9 executable."
			<< ends;

		ThrowException( "Launch", oss.str() );
	}

	string sPath = string( pszEnv ) + string( "/ds9" );
...
// +---------------------------------------------------------------------------+
void CArcDisplay::Show( void *pBuffer, int dRows, int dCols )
{
	ostringstream oss;
...
// +---------------------------------------------------------------------------+
void CArcDisplay::Show( std::string sFitsFile )
{
	ostringstream oss;
...
// |  <IN> -> dCols   - Number of columns in image buffer.                     |
// +---------------------------------------------------------------------------+
void CArcDisplay::Set( std::string sDS9Cmd, void *pBuffer, int dRows, int dCols )
{
#ifdef WIN32
...
// +---------------------------------------------------------------------------+
void CArcDisplay::RunProcess( std::string sBuf )
{
#ifdef WIN32
...
// | RunAndSaveProcess                                                         |
// +---------------------------------------------------------------------------+
// |  Creates a new process and saves PROCESS_INFORMATION ( WIN32 ) or pid_t.  |
// |                                                                           |
// |  <IN> -> sBuf - Process string to execute.                                |
// +---------------------------------------------------------------------------+
void CArcDisplay::RunAndSaveProcess( std::string sBuf )
{
#ifdef WIN32
...
// +----------------------------------------------------------------------------
void CArcDisplay::ThrowException( std::string sMethodName, std::string sMsg )
{
