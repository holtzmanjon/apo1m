...
*                         function.
* 13-Jul-2001 sds  1.6    Removed a lot of code and added
...
* 11-Jun-2002 sds  1.7A   Improved mmap management system. Still
*                         cannot recover used memory very well, however.
* 12-Jan-2002 mcba 1.7B   Using new PCI probing technique; register memory
*                         regions; spin_locks; open now enforces one
*                         process per board; removed unnecessary includes;
...
*                         wait queues; mmap determines which device is being
*                         referred to by using the file structure.
*   To do:
*   ------
*
*     - Verify with Scott and Bob that placing this driver under
*       the GPL is OK.
*     - Remove interrupts, if they are no longer needed.
*     - The technique for choosing nextValidAddress is not good when
*       multiple boards are being used.
*
*   Development notes:
*   -----------------
...
/******************************************************************************
        Global variables
******************************************************************************/
static char *astropci_name    = "astropci0";    // The name for /proc/devices
static char *astropci_version = "V1.7B 020120a";// Format: YYMMDDx
static astropci_t astropci[MAX_DEV];            // Information about each board
static u_int major;                             // The major device number 
static uint32_t nextValidStartAddress;          // The next available address 
                                                //   for DMA
static int ndev = 0;                            // Total number of boards found
static long max_elapsed_time[MAX_DEV] = {0L,};  // The max number of usecs that
...
static int astropci_ioctl(struct inode *, struct file *,
...
******************************************************************************/
static int __devinit astropci_probe       (struct pci_dev *pdev,
                                           const struct pci_device_id *ent);
static void __devexit astropci_remove     (struct pci_dev *pdev);
static void astropci_intr                 (int irq, void *dev_id, 
                                           struct pt_regs *regs);
static int  astropci_read_proc            (char *buf, char **start, 
...
init_module(void)
{
        astropci_printf("version %s; requires V1.7 firmware in the PCI board\n", astropci_version);
        ...
        if ((major = register_chrdev(0, astropci_name, &astropci_fops)) < 0) {
                astropci_printf("char device allocation failed\n");
                return major;
        }

        if (!create_proc_info_entry(astropci_name, 
                                    0, NULL, astropci_read_proc)) {
                astropci_printf("can not create proc entry\n");
                unregister_chrdev(major, astropci_name);
                major = 0;
                return -ENODEV;
        }

        nextValidStartAddress = 0;
        return pci_module_init(&astropci_driver);
}
...
cleanup_module(void)
{
        if (major > 0) {
                unregister_chrdev(major, astropci_name);
                remove_proc_entry(astropci_name, NULL);
        }
                ...
                const struct pci_device_id *ent)
{
        ...
        enable_irq(pdev->irq);
        astropci_printf(
                "successfully probed PCI device %s as dev %d, irq %d, func %d\n", 
                pdev->slot_name, dev, pdev->irq, pdev->devfn);
...
astropci_set_buffer_addresses(int dev)
{
        ...
        if (astropci_check_dsp_input_fifo(dev)) {
                ...
                astropci_printf ("timeout during set_buffer_addresses\n");
                return -ENXIO;
        }
        ...
        if (astropci_check_reply_flags(dev) != DON) {
	        astropci_printf ("check_reply_flags != DON in set_buffer_addresses\n");
	        return -ENXIO;
	}
...
astropci_wait_for_condition(int dev, int condition_to_wait_for)
{
        ...
        if (elapsed_time > max_elapsed_time[dev]) {
                astropci_printf("slept for %ld usecs\n", elapsed_time);
                max_elapsed_time[dev] = elapsed_time;
        }
...
astropci_mmap(struct file *file, struct vm_area_struct *vma)
{
        ...
        /* Assign the image buffers, we do this before the calling 
	   remap_page_range */
        if ((retval = astropci_set_buffer_addresses(dev))) {
        ...
        if ((retval = remap_page_range(vma->vm_start, 
                                       ...
                                       vma->vm_page_prot))) {
                astropci_printf("remap page range failed for dev %d\n", dev);
                goto exit;
        }

        astropci_printf
	  ("mmap %d; %u bytes: 0x%lX-0x%lX->0x%X-0x%X\n", dev,
...
astropci_read_proc(char *buf, char **start, off_t offset, int len)
{
        ...
        len += sprintf(buf + len,
                       ...
                       astropci_name, astropci_version);

        for (dev = 0; dev < MAX_DEV; dev++) {
                if (astropci[dev].have_irq) {
                        len += sprintf(buf + len,
                               "astropci%dIRQ=%d\nastropci%dMaxElapsed=%ld\n",
                                       dev, astropci[dev].pdev->irq,
				       dev, max_elapsed_time[dev]);
                }
