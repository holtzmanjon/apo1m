...
This version supports the Rev 5.1 PAL that implements hardware byte
	swapping. Operation without that PAL proceeds normally. 
...
AUX1	EQU	4		; Byte swapping or not
AUX3	EQU	0		; Diagnostic signal

; STATUS bit definitions
TIMROMBURN EQU	1		; Burning timing board EEPROM, ignore replies
BYTE_SWAP EQU	2		; = 1 to byte swap
PAL_BYTE_SWAP EQU 3		; = 1 if hardware byte swap in the pAL exists
...
; These interrupts are non-maskable, called from the host with $80xx
	...
	JSR	READ_NUMBER_OF_FRAMES_READ	; $807D
	DC	0,0				; Available
...
; Check for commands from the controller to the PCI board, FO_DEST = 1
	...
	CMP	#'IFC',B
	JEQ	<INITIALIZE_FRAME_COUNTER	; N_FRAME = 0
	CMP	#'RDI',B
...
; Process a PCI board non-vector command
	...
	CMP	#'SBS',A
	JEQ	<SET_BYTE_SWAPPING	; Is it the byte swapping command?
	CMP	#'TBS',A
	JEQ	<TEST_BYTE_SWAPPING	; Is byte swapping implemented?
...
READ_NUMBER_OF_FRAMES_READ
	MOVEP	X:N_FRAMES_0,X:DTXS	; DSP-to-host slave transmit
	NOP
	MOVEP	X:N_FRAMES_1,X:DTXS	; DSP-to-host slave transmit
	RTI
...
INITIALIZE_FRAME_COUNTER
	CLR	A
	NOP
	MOVE	A1,X:<N_FRAMES_1	; Up counter of number of frames read
	MOVE	A0,X:<N_FRAMES_0
	JMP 	<CONTROLLER_DONE	; Repy = DONE host flags
...
; Make sure the PCI address is on a 512 pixel boundary
	...
	JEQ	<SBS			;   addr = (addr + $400) .and. $FFFC00
...
; Set the image byte swapping mode
SBS	JCLR	#BYTE_SWAP,X:<STATUS,NBS ; AUX1 = 1, yes byte swap
	BSET	#AUX1,X:PDRC
	JMP	<IM_INIT
NBS	BCLR	#AUX1,X:PDRC
...
;	b. Write the pixels left over from the last 512 pixel FIFO block
...
L_FIFO	CLR	A
	MOVE	X:<IPXLS_1,Y1		; IPXLS = number of pixels written
	MOVE	X:<IPXLS_0,Y0		;  so far to the PCI bus
	MOVE	X:<NPXLS_1,A1		; Number of pixels needed to write 
	MOVE	X:<NPXLS_0,A0		;  to the PCI bus
	NOP
...
; Copy the image block (512 pixels) to DSP Y: memory
	MOVE	#IMAGE_BUFER,R0
	DO	#512,L_BUFFER
	MOVEP	Y:RDFIFO,Y:(R0)+
L_BUFFER
...
WRITE_LAST_LITTLE_BIT_OF_IMAGE
	...
	ASR	#5,A,A
	NOP		
	MOVE	A0,X:<NUM_BLOCKS	; Number of small blocks at the end
	MOVE	#>16,Y0
	MOVE	A0,Y1
	MPY	Y0,Y1,A
	NOP
...
AGAIN1	MOVEP	#$8EFA51,X:DCR0		; Start DMA with control register DE=1
	MOVEP	A1,X:DPAR		; Initiate writing to the PCI bus
	NOP
	NOP
	JCLR	#MARQ,X:DPSR,*		; Wait until the PCI operation is done
...
AGAIN2	MOVEP	A1,X:DPAR		; Write to PCI bus
	...
	JSR	<PCI_ERROR_RECOVERY
	JMP	<AGAIN2			; Try to write the sub-block again
...
AGAIN3	MOVEP	A1,X:DPAR		; Write to PCI bus
	...
	JSR	<PCI_ERROR_RECOVERY
	JMP	<AGAIN3			; Try to write the word again

; All done with writing the image to the PCI bus, so clean up
ALLDONE	BCLR	#AUX1,X:PDRC		; Restore no byte swapping
	MOVE	X:<PCI_ADDR_1,A1	; Current PCI address
	MOVE	X:<PCI_ADDR_0,A0
...
; Increment the number of frames read by one
	CLR	A
	MOVE	X:<N_FRAMES_0,A0
	MOVE	X:<N_FRAMES_1,Y0
	INSERT	#$010010,Y0,A
	INC	A
	NOP
	MOVE	A0,X:<N_FRAMES_0	; N_FRAMES_0 is 2 x 16 bits, number 
	EXTRACTU #$010010,A,A		;   of image frames read so far
	NOP
	MOVE	A0,X:<N_FRAMES_1
...
SET_BYTE_SWAPPING
	JCLR	#PAL_BYTE_SWAP,X:<STATUS,ERROR	; Test if byte swapping exists
	MOVE    X:<ARG1,A			; Get the argument, 1 to
	NOP					;    byte swap, 0 to not
	JCLR	#0,A,NOT_BS
	BSET	#BYTE_SWAP,X:<STATUS		; AUX1 = 1, yes byte swap
	JMP     <FINISH
NOT_BS	BCLR	#BYTE_SWAP,X:<STATUS		; AUX1 = 0, no byte swap
	JMP     <FINISH

; Determine whether byte swapping is implemented in the PAL
TEST_BYTE_SWAPPING
	BCLR	#3,X:PDRD			; Clear NWRFIFO, connected to AUX3
	REP	#5
	NOP
	JCLR	#0,X:PDRC,TEST_BS		; If AUX3 follows it BS may exist
NO_BS	BCLR	#PAL_BYTE_SWAP,X:<STATUS
	JMP	<ERROR

TEST_BS	BSET	#3,X:PDRD			; Clear NWRFIFO, connected to AUX3
	REP	#5
	NOP
	JCLR	#0,X:PDRC,NO_BS			; If AUX3 does not follow it
	BSET	#PAL_BYTE_SWAP,X:<STATUS	;   the BS doesn't exist
	JMP	<FINISH
...
; Parameters
STATUS		DC	4		; Execution status bits
		DC	0		; Reserved
...
N_FRAMES_1	DC 	0	; Number of frames read, MS 16-bits
N_FRAMES_0	DC 	0	; Number of frames read, LS 16-bits
