...
global CAMERAS ALTA
   set t -99
   if { $ALTA } {
     set m [$CAMERAS($id) GetImagingStatus] 
     if { $m == 4 || $m == 0 } {
        set t [$CAMERAS($id) GetTempCcd]
     }
   } else {
...
#               AVGTEMPS	-	Average temps for plotting
#               SETPOINTS	-	Cooler setpoints
global ydata ysetp TEMPWIDGET AVGTEMPS SETPOINTS tcl_platform TEMPCNTR
...
#               ysetp	-	temp plot array - setpoint
#               TEMPWIDGET	-	BLT temperature graph widget name
global ydata ysetp TEMPWIDGET tcl_platform
  set min  9999999
  set max -9999999 
  foreach i $ydata { 
     if { $i < $min } { set min $i } 
...
###    $TEMPWIDGET yconfig -scale "[expr $min-$r] [expr $max+$r] 1"
  }
}
