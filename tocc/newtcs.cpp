...
//    calibrate both directions
     ...
     if ( (axis == 'x') || (axis == 'X') ) {
       ...
       sprintf(outbuf,"New encoder counts per degree - CCW = %f",value1);
       writeline(outbuf,0);
       ...
       sysGlobal->x_geartrain_normal = !sysGlobal->x_geartrain_normal;
       sprintf(outbuf,"New encoder counts per degree - CW = %f",value2);
       writeline(outbuf,0);
  ...
  // Reset network clock so we don't immediately close!
...
unsigned calibrate_az_steps(double &newvalue)
#endif
 {
   ...
   long startcount = labs(tcs_return_encoder_position('x'));
...
//  New way uses km command just like home telescope
   ...
   long count = labs(tcs_return_encoder_position('x'));
#ifndef OLDCAZ
...
//   pc38_send_commands("az kl;");
   ...
   // how many steps?
   long count = labs(tcs_return_step_position('z'));
   sprintf(outbuf, "Steps around = %ld", count);
   writeline(outbuf,0);
...
unsigned tcs_home_secondary()
 {
   ...
   char buffer[81];
   sprintf(buffer, "at lf; ac%u; vl%u; km;",
   ...
   if (sysGlobal->tuv_type == 1)
     {
       sprintf(buffer, "au lf; ac%u; vl%u; km;",
	       sysGlobal->tuv_acceleration,
	       sysGlobal->tuv_max_velocity);
       pc38_send_commands(buffer);

       sprintf(buffer, "av lf; ac%u; vl%u; km;",
	       sysGlobal->tuv_acceleration,
	       sysGlobal->tuv_max_velocity);
       pc38_send_commands(buffer);
       u_done = FALSE;
   ...
   do
       ...
       if (!u_done)
	 {
       ...
       if (!v_done)
	 {
     pc38_send_commands("au lp0; av lp0;");
   ...
   //**************************************
	 ...
	 */
	 ...
	 /*
	     ...
	     position of the z axis.  We will look at the last know
...
//                   sysGlobal->z_max_velocity, -autoGlobal->z_axis_pos);
    ...
    */
    ...
    if (sysGlobal->z_axis_enabled)
	 ...
	 pc38_send_commands(x_command);
       ...
       {
	 sprintf(x_command, ECHOCH"ax vl%ld; ay vl%ld;",
		 (sysGlobal->x_max_velocity / 4),
		 (sysGlobal->y_max_velocity / 4));
	 pc38_send_commands(x_command);
       }
    ...
    /*
	 ...
	 state 0:  off home in the positive direction
	       ...
	       3:  loop until done
	       4:  do nothing
	       5:  off home (in home sensor hesterisis timer)
    */
   ...
   // load the step counter/encoders with 0
    ...
    get_all_times_at(d, t, timeRec);
    if (!sysGlobal->mount_type){
      G->current_mean_ra = slaDranrm(timeRec.last - autoGlobal->home_ha*DH2R);
...
unsigned tcs_move_secondary_steps(long t_axis, long u_axis, long v_axis)
 {
   ...
   if (t_axis)
     {
     sprintf(buffer, "at lf; ac%u; vl%u; mr%ld; gd; id;",
	       sysGlobal->tuv_acceleration,
	       sysGlobal->tuv_max_velocity,
	       t_axis);
     pc38_send_commands(buffer);
   ...
   if (sysGlobal->tuv_type == 1)
     {
       if (u_axis)
	 {
	 sprintf(buffer, "au lf; ac%u; vl%u; mr%ld; gd; id;",
		   sysGlobal->tuv_acceleration,
		   sysGlobal->tuv_max_velocity,
		   u_axis);
	 pc38_send_commands(buffer);
	 u_done = FALSE;
       ...
       if (v_axis)
	 {
	 sprintf(buffer, "av lf; ac%u; vl%u; mr%ld; gd; id;",
		   sysGlobal->tuv_acceleration,
		   sysGlobal->tuv_max_velocity,
		   v_axis);
	 pc38_send_commands(buffer);
...
//   #endif
   ...
   if (x_axis)
       ...
       if (sysGlobal->x_encoder_installed)
	 {
	   ...
	   sprintf(x_commandb, ENECHOCH"ax; ca; hv%u; hg%u; hd%u; ip;",
   ...
   if (y_axis)
       ...
       if (sysGlobal->y_encoder_installed)
	 {
	   ...
	   // set up final move string
	   sprintf(y_commandb, ENECHOCH"ay; ca; hv%u; hg%u; hd%u; ip;",
...
*/
   ...
   if (epoch != G->current_mean_epoch && !G->telescope_at_home) {
     sprintf(outbuf,"Calculating \"to\" SLALIB parameters");
     writeline(outbuf,1);
     slaMappa(epoch, timeRec.mjd_tt, G->mean_to_app_parms);
   }
   save_mtoa_parms_to =
   ...
   slaAoppa(timeRec.mjd_utc, G->ut1_minus_utc, G->longitude,
	     G->latitude, sysGlobal->altitude, G->polor_motion_x,
	     G->polor_motion_y, out_temp, G->current_barometer,
	     G->current_humidity, eff_wavelength, tlr,
	     G->app_to_obs_parms);
#endif
 ...
 }   // end of if not quick block
 else
 {
   move_time = 3 ;
 } // end of quick if-then-else
...
*/
   ...
   writeline(outbuf,1);
   G->nmove = 1;
   unsigned status = tcs_move_telescope_steps(steps_x, steps_y, steps_z,
					       &TotalMoveTimer);

   // Was there an error? If so, deinit the telescope, but finish the dome move
   if (status) {
       if ((status == TCSERR_MOVETIMEOUT) && move_the_dome) {
	   while (!finished)
	     ocs_dome_finished(finished);
...
unsigned tcs_dust(int open)
 {
   ...
   writeline("Sending the telescope  to altitude 85 ...",0);
   char y_command[81];

   sprintf(y_command, ECHOCH"ay vl%ld; ma%ld; gd; id;",
	    sysGlobal->y_max_velocity,(long)(6*sysGlobal->y_steps_degree));
   pc38_send_commands(y_command);
...
unsigned tcs_reset_home_position(BOOL sessiononly)
 {
   ...
   if (sessiononly)
     ...
     writeline(outbuf,0);
     writetoccscf(autoGlobal);
     sprintf(outbuf,"Update home positions: %lf %lf %lf %lf");
     writelog(outbuf,7);
     return(0);
   }
 }
...
unsigned tcs_telescope_fill()
 {
   ...
   if (sysGlobal->z_axis_enabled) {
     sprintf(z_command, ECHOCH"az vl%ld; ma%ld; gd; id;",
	    sysGlobal->z_max_velocity,(long)(-90*sysGlobal->z_steps_degree));
//    sysGlobal->z_max_velocity,(long)(70*sysGlobal->z_steps_degree));
     pc38_send_commands(z_command);
   }
   ...
   int wait;
#define Havefillsensor
#ifdef Havefillsensor
   wait = 20;
   sprintf(outbuf,"Waiting for fill to complete, or 20 minutes, whichever is less.\n"
" All operations (including status) will be suspended...",wait);
   writeline(outbuf,0);
#else
   if (dtime*24. > sysGlobal->ccd_hold_time ) {
     wait = 10;
   } else {
     wait = ceil(dtime / sysGlobal->ccd_hold_time * 10);
     wait = (wait>3 ? wait : 3);
   }
   sprintf(outbuf,"Waiting %d minutes for fill to complete.\n All operations (including status) will be suspended...",wait);
   writeline(outbuf,0);
#endif
...
//   Delay(wait*60*1000L);
   ...
   FillTimer.NewTimerSecs(wait*60);
#ifdef Havefillsensor
   ...
   if (FillTimer.Expired()) {
     ...
     writeline(outbuf,0);
   } else if (retry==0 && FillTimer.RemainingTimeInSecs() > (wait-5)*60 ) {
